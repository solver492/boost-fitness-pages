    ...
    <header className="sticky top-[32px] sm:top-[36px] z-40 bg-zinc-950/90 backdrop-blur border-b border-zinc-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-3">
        ...
        <nav className="hidden md:flex items-center gap-7 text-sm font-medium text-zinc-300">
          <Link to="/" className="hover:text-orange-400">{t("nav.home")}</Link>
          <Link to="/boutique" className="hover:text-orange-400">{t("nav.shop")}</Link>
          <a href="#contact" className="hover:text-orange-400">{t("nav.contact")}</a>
        </nav>
        <div className="flex items-center gap-2">
          <LanguageSwitcher compact />
          <a href="tel:+212600000000" className="hidden md:inline-flex items-center gap-2 px-3 py-2 rounded-md bg-zinc-900 hover:bg-zinc-800 text-white text-sm">
      ...
      {open && (
        <div className="md:hidden border-t border-zinc-900 bg-zinc-950 px-4 py-3 flex flex-col gap-3 text-sm">
          <Link to="/" onClick={() => setOpen(false)} className="text-zinc-200">{t("nav.home")}</Link>
          <Link to="/boutique" onClick={() => setOpen(false)} className="text-zinc-200">{t("nav.shop")}</Link>
  ...
  return (
    <footer id="contact" className="bg-zinc-950 border-t border-zinc-900 text-zinc-400 text-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div>
          <h4 className="text-white font-bold mb-3">{t("footer.nav")}</h4>
          <ul className="space-y-2">
            <li><Link to="/" className="hover:text-orange-400">{t("nav.home")}</Link></li>
            <li><Link to="/boutique" className="hover:text-orange-400">{t("nav.shop")}</Link></li>
          </ul>
        </div>
        <div>
          <h4 className="text-white font-bold mb-3">{t("footer.contact")}</h4>
          <ul className="space-y-2">
